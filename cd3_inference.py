...
def inference(dataloader, model, anchors, device, patch_size, down_factor, detect_thresh=0.1, nms_thresh=0.4):
    ...
    with torch.inference_mode():
        ...
        for clas_pred, bbox_pred, x, y in zip(class_pred_batch, bbox_pred_batch, x_coordinates, y_coordinates):
            ...
            # TODO: apply NMS on complete results
    return detections

def process(slide_dir, model_path, level, patch_size, visualize=False):
    ...
    for file in os.listdir(slide_dir):
        ...
        detections = inference(dl, model, anchors, device, patch_size, slide.level_downsamples[level])
...
def main():
    ...
    parser.add_argument('--model_path', type=str, help='Model weights', default='all', choices=['hnscc', 'nsclc', 'tnbc', 'gc', 'pan_tumor'])
    ...
    process(slide_dir=args.slide_dir, model_path=args.model_path, level=args.level, patch_size=args.patch_size)
